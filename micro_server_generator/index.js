...
const getAttributeForElement = (_element) => {
  ...
  const parentDir = path.dirname(_element.layer.selectedElement.path);
  // eslint-disable-next-line global-require
  const baseDir = require('path').resolve(parentDir, '..');
  const traitName = path.basename(baseDir);
  const attribute = {
...
const getRandomRarity = (_rarityOptions) => {
  ...
  for (let i = 0; i <= _rarityOptions.length; i += 1) {
...
const createDna = (_layers, _rarity) => {
  const randNum = [];
  const _rarityWeight = rarityWeights.find((rw) => rw.value === _rarity);
  _layers.forEach((layer) => {
    let num = Math.floor(
      Math.random() * layer.elementIdsForRarity[_rarity].length,
    );
    if (_rarityWeight && _rarityWeight.layerPercent[layer.id]) {
      // if there is a layerPercent defined, we want to identify which dna to actually use here
      // (instead of only picking from the same rarity)
      const _rarityForLayer = getRandomRarity(
...
const getRarity = (_editionCount) => {
  if (!rarityForEdition) {
    ...
    rarityWeights.forEach((rarityWeight) => {
      for (let i = rarityWeight.from; i <= rarityWeight.to; i += 1) {
...
const startCreating = async () => {
  console.log('########################################');
  console.log('# Generative Art by Elbrus Smart Chain');
  ...
  while (editionCount <= editionSize) {
    ...
    await Promise.all(loadedElements).then((elementArray) => {
      ...
      const nftMetadata = generateMetadata(newDna, editionCount, attributesList);
